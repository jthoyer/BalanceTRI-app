...
const eventTypes=['Triathlon','Swim','Bike','Run','SwimRun'];
...
function openEditScreen(raceId){
  ...
  setEventTypeField(f,race.eventType);
...
function setEventTypeField(form,value){
  const select=form.eventType,other=form.querySelector('.event-type-other-field'),otherInput=form.eventTypeOther;
  const known=eventTypes.includes(value);
  select.value=known||!value?value:'Other';
  otherInput.value=known||!value?'':value;
  other.classList.toggle('hidden',select.value!=='Other');
}
function readEventType(form){
  return form.eventType.value==='Other'?form.eventTypeOther.value.trim():form.eventType.value;
}
function wireEventTypeField(form){
  form.eventType.onchange=()=>{
    form.querySelector('.event-type-other-field').classList.toggle('hidden',form.eventType.value!=='Other');
    if(form.eventType.value==='Other')form.eventTypeOther.focus();
  };
}
...
wireEventTypeField($('#raceForm'));wireEventTypeField($('#editForm'));
$('#athleteFilter').oninput=e=>{state.filter=e.target.value;persist();render()};$('#eventTypeFilter').onchange=e=>{state.eventTypeFilter=e.target.value;persist();render()};$('#heroAddButton').onclick=()=>{if(editingId)closeEditScreen();toggleAdd(true);$('#addPanel').scrollIntoView({behavior:'smooth',block:'start'})};$('#closeAddButton').onclick=()=>toggleAdd(false);$('#cancelAddButton').onclick=()=>toggleAdd(false);$('#raceForm').addEventListener('submit',async e=>{e.preventDefault();const f=new FormData(e.target);const submitBtn=e.target.querySelector('.primary-button');submitBtn.disabled=true;let url=f.get('url').trim();if(url&&!/^https?:\/\//i.test(url))url='https://'+url;try{await addRace({name:f.get('name').trim(),date:f.get('date'),location:'Location TBC',url,events:f.get('events').split(',').map(x=>x.trim()).filter(Boolean),eventType:readEventType(e.target),clubFocus:f.get('clubFocus')?'Y':'N'});e.target.reset();e.target.querySelector('.event-type-other-field').classList.add('hidden');toggleAdd(false);await loadRaces()}catch(err){alert('Could not add race: '+err.message)}finally{submitBtn.disabled=false}});
$('#editCancelButton').onclick=closeEditScreen;$('#editBackButton').onclick=closeEditScreen;$('#editForm').addEventListener('submit',async e=>{e.preventDefault();const raceId=editingId;if(!raceId)return;const f=new FormData(e.target);const submitBtn=e.target.querySelector('.primary-button');submitBtn.disabled=true;let url=f.get('url').trim();if(url&&!/^https?:\/\//i.test(url))url='https://'+url;const name=f.get('name').trim();try{await updateRace(raceId,{name,date:f.get('date'),url,events:f.get('events').split(',').map(x=>x.trim()).filter(Boolean),eventType:readEventType(e.target),clubFocus:f.get('clubFocus')?'Y':'N'});closeEditScreen();await loadRaces();showToast(`${name} updated`)}catch(err){alert('Could not save changes: '+err.message)}finally{submitBtn.disabled=false}});
